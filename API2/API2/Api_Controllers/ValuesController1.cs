...
namespace API2.Api_Controlers
{
    public class FilesDataController : ApiController
    {
                ...
                try
                {
                    foreach (var fileName in HttpContext.Current.Request.Files.AllKeys)
                    {
                        ...
                        if (file != null)
                        {
                            ...
                            ModelContr.SaveFileInDB(fileDTO);
        ...
        public Object DownloadFile(String uniqueName)
        {
            ...
            var rootPath = System.Web.HttpContext.Current.Server.MapPath("~/UploadFiles");

            //Find File from DB against unique name
            var fileDTO = ModelContr.GetFileByUniqueID(uniqueName);
